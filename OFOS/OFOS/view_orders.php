    ...
    <table>
        ...
        if ($result && $result->num_rows > 0) {
            while($row = $result->fetch_assoc()) { 
                $sql = "select * from order_details left join tbl_restaurants on tbl_restaurants.R_ID = order_details.res_id where order_details.order_id = ".$row['id'];
                $res = $conn->query($sql);
            ?>
        <tr>
            <td><?php echo $i; ?></td>
            ...
            <td>
                <?php 
                    while($detail = mysqli_fetch_assoc($res)){
                        ?>
                        <div><?php echo $detail['food_name']?> - <?php echo $detail['qty']?> (<?php echo $detail['name']?>)</div>
                        <?php
                    }
                ?>
            </td>
